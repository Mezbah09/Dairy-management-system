...
class FarmerController extends Controller
{
    ...
    public function loadDate(Request $r)
    {
        ...
        $farmer1->old = FarmerReport::where(['year' => $r->year, 'month' => $r->month, 'session' => $r->session, 'user_id' => $r->user_id])->count() > 0;
        $report = FarmerReport::where(['year' => $r->year, 'month' => $r->month, 'session' => $r->session, 'user_id' => $r->user_id])->first();
        $sellitem = Sellitem::where('user_id', $r->user_id)->where('date', '>=', $range[1])->where('date', '<=', $range[2])->get();
        $milkData = Milkdata::where('user_id', $r->user_id)->where('date', '>=', $range[1])->where('date', '<=', $range[2])->get();
        $snfFats = Snffat::where('user_id', $r->user_id)->where('date', '>=', $range[1])->where('date', '<=', $range[2])->get();
        ...
        $ledgers = Ledger::where('user_id', $r->user_id)->where('date', '<=', $range[2])->orderBy('date', 'asc')->orderBy('id', 'asc')->get();
        // dd($ledgers,$range);
        ...
        return view('admin.farmer.detail.index', compact('snfFats', 'milkData', 'data', 'center', 'farmer1', 'milk_rate', 'prev', 'closing'));
    }
    ...
    public function dueLoad(Request $request)
    {
        ...
        $balance = DB::selectOne(
            'select
            (select sum(amount) as data from ledgers where user_id=' . $user->id . ' and type=2 and date>=20780401) as d,
            (select sum(amount) as data from ledgers where user_id=' . $user->id . ' and type=1 and date>=20780401) as c'
        );
        $due = $balance->c - $balance->d;
